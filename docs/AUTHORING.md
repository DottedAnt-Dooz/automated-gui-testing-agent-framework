...
3. Generate from the template using the shared runtime. Dot-source `Framework/GeneratedScriptRuntime.ps1`, then use `Get-AGTARuntimeHelp -Name <helper>` for signatures and source paths. The runtime imports `ArtifactAssertions.ps1`; searching only the main runtime file cannot determine whether `Assert-ArtifactPrefix` exists. Run `Assert-AGTAGeneratedScriptPreflight -ScriptPath <file> -TestCaseCsv <csv> -PotatoCliPath <cli>` before any GUI action; the template and API authoring tools do this automatically. Execute once, fix concrete failures, then rerun after behavior changes. Do not rerun only to polish reporting.
