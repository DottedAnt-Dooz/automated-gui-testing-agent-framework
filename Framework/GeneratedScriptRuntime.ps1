...
function Invoke-PotatoJson {
    ...
    $raw = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $context.PotatoCliPath $Command @Arguments
    $watch.Stop()
...
function Assert-PotatoFound {
    ...
    if (-not (Test-PotatoFound -Result $Result)) {
        throw $Message
    }
}
...
function Invoke-RecordedStep {
    ...
    try {
        & $Body ([ref]$commands) ([ref]$evidence)
        return New-StepResult -StepIndex $StepIndex -Action $step.Action -ExpectedResult $step.'Expected Result' -Status 'PASS' -Evidence $evidence -Commands $commands
    }
    catch {
        return New-StepResult -StepIndex $StepIndex -Action $step.Action -ExpectedResult $step.'Expected Result' -Status 'FAIL' -Evidence $evidence -Commands $commands -ErrorObject $_.Exception.Message
    }
}
...
function Complete-AGTAGeneratedTest {
    ...
    $ok = ($summary.failed -eq 0 -and ($AllowSkipped -or $summary.skipped -eq 0) -and $summary.total -eq $context.Steps.Count)
    $final = [ordered]@{
        ok = $ok
